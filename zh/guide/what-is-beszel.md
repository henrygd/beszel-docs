...
- **简单**: 设置简单，只需极少的手动配置。
- **警报**: 可为大多数指标配置警报。支持多种通知服务。
- **Docker 统计**: 跟踪每个容器的 CPU、内存和网络使用历史。
- **网络监控**: 直接通过代理监控响应时间和中断情况。
- **S.M.A.R.T.**: 磁盘健康数据和驱动器故障通知。
- **多用户**: 用户管理自己的系统。管理员可以在用户之间共享系统。
- **OAuth / OIDC**: 支持多种 OAuth2 提供程序。可以禁用密码验证。
- **自动备份**: 将数据保存到磁盘或 S3 兼容存储，并从中恢复。
- **电池**: 主机系统电池电量。
...
- **Docker 卷使用率** - 每个卷的大小。需手动启用，参见 [`DOCKER_VOLUME_INTERVAL`](./environment-variables#docker-volume-interval)。
- **网络使用率** - 主机系统和容器。
- **负载平均值** - 主机系统。
- **温度** - 主机系统传感器。
- **风扇转速** - 主机系统传感器（Linux，通过 `/sys/class/hwmon`）。
- **GPU 使用率 / 功耗** - Nvidia, AMD, 和 Intel。
- **电池电量** - 主机系统和部分外设。
- **容器** - 所有正在运行的 Docker / Podman 容器的状态和指标。
- **S.M.A.R.T.** - 主机系统磁盘健康状况（包括 eMMC 磨损/寿命终止状态，以及可通过 sysfs 获取的 Linux mdraid 阵列健康状况）。
- **ZFS** - 存储池容量、使用量、健康状态、I/O 吞吐量、清理状态以及按数据集的使用量。
- **网络监控** - 从代理探测 ICMP、TCP、HTTP 和 DNS 的响应时间与丢包情况。请参阅[网络监控](./network-monitors)。
