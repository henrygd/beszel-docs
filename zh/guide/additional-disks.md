...
::: tip ZFS
ZFS 存储池和数据集会自动监控 — 无需将它们添加到 `EXTRA_FILESYSTEMS`。如果您确实添加了 ZFS 挂载点，其使用量将从 `zfs list` 报告（与存储池一致），而不是挂载的文件系统，并且不会显示 diskstats I/O 警告。请参阅 [ZFS 监控](./storage-pools#zfs)。
:::
