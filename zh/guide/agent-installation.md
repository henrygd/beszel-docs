...
::: warning 无根 Docker / Podman
无根容器无法访问大多数主机级资源。容器统计信息在使用[正确的套接字路径和 cgroup 委派](./common-issues.md#无根代理的-docker-统计信息缺失)时可以正常工作，但温度传感器、风扇、电池、GPU、ZFS、S.M.A.R.T. 和 systemd 服务监控可能不可用，或需要额外配置。

如果您需要这些指标，请考虑改为将[二进制代理](#二进制文件)作为 systemd 服务运行。
:::
